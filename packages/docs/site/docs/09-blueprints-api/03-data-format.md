...
JSON files can be tedious to write and easy to get wrong. To help with that, Playground provides a [JSON schema](https://wp-playground.khulnasoft.com/blueprint-schema.json) file that you can use to get autocompletion and validation in your editor:

```js
{
	"$schema": "https://wp-playground.khulnasoft.com/blueprint-schema.json",
	"landingPage": "/wp-admin/",
	// ...
}
```
